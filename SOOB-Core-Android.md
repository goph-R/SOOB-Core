...
- **Game identity: done** — as `app.lua` beside `assets.lua`, read by all three
  ...
  its `<title>` and PWA manifest from it, and the Android app module has **no
  Kotlin at all** — `SoobActivity` is concrete, so the manifest names
  `net.dynart.soob.SoobActivity` directly and a game needs no subclass. What
  stays per-game in the app module is what Play requires there: `applicationId`
  and the launcher icon; even the label comes from `app.lua`.
...
### Since then (2026-09-19)

Four structural changes, all driven by making a second game shippable:

- **The package is `net.dynart.soob`**, not `info.dynart.soob`. The JNI layer
  encodes it in symbol names, so this touched three textual forms: the Kotlin
  and manifests, `FindClass("net/dynart/soob/Host")`, and all 14
  `Java_net_dynart_soob_Lua_*` exports plus the `JNIFN` macro that generates
  them.
- **The gradle scripts are portable.** `soobApp.gradle` and `syncGame.gradle`
  locate their siblings relative to themselves rather than to `rootProject`,
  and the `:soob-player` dependency falls back to the coordinate when that
  project path is absent — so both work from a game repo's own `android/`
  root. `soob-player` gained `group`/`version`, without which a composite build
  has nothing to substitute.
- **`syncGame` copies SOOB-Core's `scripts/engine`** into the bundle. Those
  modules are generated by the desktop builds and gitignored, so a freshly
  cloned game had none and `require "engine.scene"` threw at boot after a
  perfectly successful build. They now go straight into the packaged assets,
  so an APK builds on a machine with no desktop toolchain and the game repo is
  never written to.
- **Find5 ships from `Find5/android`.** `app/` here is a neutral dev harness —
  see [Template / multi-game](#template--multi-game).
...
SOOB-Core-Android/
  ...
  gradle/soobApp.gradle                           everything an app module needs that is not identity
  gradle/syncGame.gradle                          the bundle copy + generated label/colour resources

  app/build.gradle                                com.android.application — the DEV HARNESS, not a
                                                  shipping game: applicationId "net.dynart.soob.demo",
                                                  versionCode/Name. Point it anywhere with
                                                  -PsoobGame=../MyGame
  app/src/main/AndroidManifest.xml                launcher intent naming SoobActivity directly
...
**Settled: option 2.** A game's app module lives in **the game's own repo**, in
an `android/` folder, pulling the player in with
`includeBuild '../SOOB-Core-Android'` — exactly how **CoolFox consumes
LisaEngine**. The game's Android identity sits beside its desktop `main.cpp`,
and player fixes reach every game without a merge.

Find5 ships that way from `Find5/android`, and
[`SOOB-Core-Template`](https://github.com/goph-R/SOOB-Core-Template) carries the
module ready-made as its `android/` folder, so a clone-and-rename produces an
APK. `app/` in this repo stayed behind as a neutral dev harness
(`net.dynart.soob.demo`) that puts any bundle on a device via `-PsoobGame`.

A second module beside `app/` here still works — same `build.gradle`, same
`apply from` — but it puts one game's store identity in the player's repo, so
prefer the game-repo module. Either way the library is consumed, never forked.

### Game identity belongs in the bundle

**Implemented — this is `app.lua` now; the section below is why.** The shipped
shape is a *flat* table, not the nested `app = { … }` sketched below, and it
also carries `description` (the web app manifest) and `background` (the clear
colour, and the Android adaptive-icon background).
